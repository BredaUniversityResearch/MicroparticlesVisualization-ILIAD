...
public class MapScale : MonoBehaviour
{
    ...
    /// </summary>
    [Tooltip("The top-level GeoReference.\nThis is used to convert Unity coordinates to globe coordinates.")]
    ...
    /// </summary>
    [Tooltip("The globe anchor for the camera.\nThis is used to compute the height of the camera in meters.")]
    public CesiumGlobeAnchor CameraGlobeAnchor;

    /// <summary>
    /// Unity scene camera. Used to convert screen-space values to view space.
    /// </summary>
    [Tooltip("The camera used to convert screen-space values to view-space.\nIf not specified, the main camera is used.")]
    public Camera Camera;

    /// <summary>
    /// The width of the UI element in screen-space.
    /// </summary>
    [Tooltip("The screen-space width of the UI image that represents the map scale indicator.")]
    public float ScreenSpaceWidth;

    /// <summary>
    /// (Optional) If this points to a valid UI transform, it will be used to determine the screen-space width of the UI element that determines the width
    /// in screen space.
    /// </summary>
    [Tooltip("(Optional) The RectTransform of the UI image that represents the map scale indicator.\nIf not specified, the Screen Space Width value is used.")]
    public RectTransform MapScaleUITransform;

    /// <summary>
    /// The UI Text element to update with the scale value.
    /// </summary>
    [Tooltip("The UI text label used to show the global units.")]
    ...
    /// <summary>
    /// Updating the label has to happen in the LateUpdate to ensure any transform changes
    /// to the camera have propagated to the camera's globe anchor.
    /// </summary>
    void LateUpdate()
    {
        if (Georeference == null || CameraGlobeAnchor == null || Label == null) return;
        if (MapScaleUITransform != null)
            ScreenSpaceWidth = MapScaleUITransform.rect.width;
    ...
    private string Distance(double meters)
    {
        ...
        string m = Math.Abs(meters) switch
        {
        ...
        string nm = (meters / 1852.0f).ToString(format) + " nm";
    ...
    private double GetDistanceAtSurface()
    {
        ...
        var p1 = new Vector3(Camera.pixelWidth / 2.0f + ScreenSpaceWidth, Camera.pixelHeight / 2.0f, 1.0f); // 1/2 from width of UI element.
